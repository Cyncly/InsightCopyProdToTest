...
GO
-- Version : $Id: A020_scDBA_utl Set Database Options.sql,v 1.67 2022/06/15 14:14:04 US01\moldvl Exp $  
BEGIN TRY
    ...
    -- Bug 700356: 3CADNext processing error, Database name is not allowed with a table-valued parameter,
    IF DB_NAME() LIKE N'%[.]%'
        THROW 50000, 'Due to Microsoft bug, Insight does not support SQL database names with these special characters: [.]', 1;
    ...
    IF CAST(SERVERPROPERTY('ProductMajorVersion') as int) < 16 -- SQL 2022
       THROW 50000, 'Running this script is only supported on SQL Server 2022 and higher.', 1; 
    /*
    ...
    */
    ...
    (
        SELECT
            ...
            (
                ...
                --    WHEN pmv.ProductMajorVersion = 16 AND [compatibility_level] != 160 -- SQL 2022
                --    THEN N', COMPATIBILITY_LEVEL = 160'
                --END + NCHAR(13)
                CASE WHEN [compatibility_level] != 160 THEN N', COMPATIBILITY_LEVEL = 160' + NCHAR(13) END
                ...
                ,CASE WHEN @New_Broker = 1 THEN N', NEW_BROKER' + NCHAR(13) END
                ...
                ,CASE WHEN is_query_store_on = 0 THEN N', 
                    QUERY_STORE = ON 
                    (
    ...
    -- https://learn.microsoft.com/en-us/sql/t-sql/statements/alter-database-scoped-configuration-transact-sql?view=sql-server-ver16&f1url=%3FappId%3DDev15IDEF1%26l%3DEN-US%26k%3Dk(ALTER_DATABASE_SCOPED_CONFIGURATION_TSQL)%3Bk(sql13.swb.tsqlresults.f1)%3Bk(sql13.swb.tsqlquery.f1)%3Bk(MiscellaneousFilesProject)%3Bk(DevLang-TSQL)%26rd%3Dtrue
    ALTER DATABASE SCOPED CONFIGURATION SET QUERY_OPTIMIZER_HOTFIXES = ON -- enables hotfixes since RTM release
    ...
    IF OBJECT_ID('inResponse.ServiceServiceNotifications', 'U') IS NOT NULL
    BEGIN
        EXECUTE('
        DELETE ssn
        FROM inResponse.[Services] svc
        JOIN inResponse.ServiceServiceNotifications ssn ON ssn.svcID = svc.svcID
        WHERE svc.istID IN 
        (
            1 -- Standard
    ...
    IF EXISTS(SELECT * FROM sys.server_event_notifications n WHERE [name] = 'inSightDeadLockEventNotification')
        DROP EVENT NOTIFICATION inSightDeadLockEventNotification ON SERVER

    DECLARE @SQL1 varchar(max) = 
    (
        SELECT STRING_AGG(CONCAT('END CONVERSATION ''', [conversation_handle], CAST(''' WITH CLEANUP' as varchar(max))), '; ')
        FROM sys.conversation_endpoints ce
